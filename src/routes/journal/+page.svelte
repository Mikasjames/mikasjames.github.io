<script lang="ts">
	...
	const totalHabitLogs = $derived(
		Object.values(habitLogsByDate).reduce(
	...
	const averageHappiness = $derived.by(() => {
		...
		const total = ratedEntries.reduce(
			(sum, entry) => sum + (entry.happinessRating ?? 0),
			0,
		);
...
<div class="min-h-screen bg-[#09090b] pt-20 pb-12 px-4 md:px-8">
	...
	{:else if user}
		<div class="relative mx-auto max-w-7xl">
			<div
				class="mb-6 flex flex-col gap-5 border-b border-zinc-800/80 pb-6 lg:flex-row lg:items-end lg:justify-between"
			>
				<div>
					<div class="mb-2 flex items-center gap-2">
						...
						></span>
						<p
							class="font-mono text-xs text-accent-400 tracking-widest uppercase"
						>
							&gt;_ private journal
						</p>
					</div>
					<h1
						class="text-3xl font-bold tracking-tight text-zinc-100 md:text-4xl"
					>
						Thoughts & Records
					</h1>
					<p
						class="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-500"
					>
						A private archive for daily notes, mood signals, and
						habit context.
					</p>
				</div>
			...
			{#if errorMsg}
				...
				>
					...
					<p
						class="text-[11px] text-zinc-550 pt-2 leading-relaxed border-t border-red-900/20"
					>
						Please make sure Firestore Security Rules allow read
						access to the <code
							class="bg-zinc-900/80 px-1 py-0.5 rounded text-red-300"
							>journal</code
						>
						collection for your user UID:
						<code
							class="bg-zinc-900/80 px-1 py-0.5 rounded text-zinc-300"
							>{user.uid}</code
						>.
					</p>
				</div>
			{:else}
				<div class="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
					<div
						class="rounded-xl border border-zinc-850/70 bg-zinc-950/35 px-4 py-3"
					>
						<p
				...
				>
					<div class="space-y-4 lg:sticky lg:top-24 lg:col-span-4">
						<div
							class="rounded-2xl border border-zinc-850/70 bg-surface-900/45 p-3 shadow-2xl shadow-black/20"
						>
							<div class="relative">
								<div
									class="absolute inset-y-0 left-3 flex items-center pointer-events-none text-zinc-600"
								>
									<svg
										class="w-4 h-4"
										fill="none"
						...
						<div class="flex gap-2">
							<div class="flex-1">
								<label
									for="journal-date-from"
									class="mb-1 block text-[10px] font-mono uppercase tracking-widest text-zinc-600"
									>From</label
								>
								<input
						...
						<div
							class="max-h-[calc(100vh-20rem)] space-y-2 overflow-y-auto pr-1"
						>
							{#if filteredEntries.length === 0}
								<div
									class="rounded-xl border border-dashed border-zinc-850 bg-zinc-900/10 py-12 text-center"
								>
									<p
									...
									<p class="text-zinc-650 text-xs mt-1">
										Try another search or write a new entry.
									</p>
								</div>
							{:else}
								{#each filteredEntries as entry (entry.id)}
									<button
										...
										class="group w-full rounded-xl border p-3 text-left transition-all duration-200 {selectedEntryId ===
										entry.id
									...
									>
										<div class="flex gap-3">
											{#if entry.coverImage}
												<div
													class="h-16 w-16 shrink-0 overflow-hidden rounded-lg border border-zinc-800/80 bg-zinc-950"
												>
													<img
											...
											<div class="flex-1 min-w-0">
												...
												>
													<h3
														class="flex-1 truncate text-sm font-semibold text-zinc-200 group-hover:text-accent-300"
													>
														{entry.title ||
															"Untitled Entry"}
													</h3>
													<span
														class="mt-0.5 shrink-0 text-[10px] font-mono text-zinc-550"
													>
														{getEntryDate(
															entry,
														).toLocaleDateString(
												...
												<div
													class="mt-3 flex items-center justify-between border-t border-zinc-850/50 pt-2.5"
												>
													<span
													...
													>
														{#if entry.happinessRating}
															<span
																class="rounded-full border px-2 py-0.5 text-[10px] font-semibold {happinessTone(
														...
														{/if}
														<span
															class="text-[10px] font-medium text-accent-400/80 group-hover:text-accent-300"
															>Open</span
														>
													</div>
												</div>
												{#if entryHabitLogs(entry).length}
													...
													>
														{#each entryHabitLogs(entry) as log (log.id)}
															<span
																class="rounded-full border border-zinc-800 bg-zinc-950/50 px-2 py-0.5 text-[10px] text-zinc-500"
															>
																{log.emoji}
																{log.habitName}
															</span>
					...
					<div class="lg:col-span-8">
						{#if selectedEntry}
							<article
								class="overflow-hidden rounded-2xl border border-zinc-850/70 bg-surface-900/55 shadow-2xl shadow-black/25"
							>
								{#if selectedEntry.coverImage}
									<div
										class="relative aspect-[16/7] w-full overflow-hidden border-b border-zinc-800/80 bg-zinc-950"
									>
										<img
										...
										/>
										<div
											class="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-surface-900 to-transparent"
										></div>
									</div>
								{/if}
								...
								<div class="p-5 md:p-8">
									<div
										class="border-b border-zinc-800/80 pb-6 mb-7"
									>
										<div
											class="mb-4 flex flex-wrap items-center gap-2 text-xs text-zinc-500"
										>
											<span
												class="rounded-full border border-zinc-800 bg-zinc-950/45 px-2.5 py-1 font-mono text-zinc-500"
											>
												{formatMonth(
													getEntryDate(selectedEntry),
												)}
											</span>
											<time
												class="flex items-center gap-1 rounded-full border border-zinc-800 bg-zinc-950/45 px-2.5 py-1 font-mono"
											>
												<svg
													class="w-3.5 h-3.5"
													fill="none"
											...
											<span
												class="rounded-full border border-zinc-800 bg-zinc-950/45 px-2.5 py-1 font-mono"
											>
										...
										<h2
											class="max-w-3xl text-3xl font-bold tracking-tight text-zinc-100 md:text-4xl"
										>
											{selectedEntry.title ||
												"Untitled Entry"}
										</h2>
										<p
											class="mt-3 text-sm font-mono text-zinc-600"
										>
											{formatDate(
												getEntryDate(selectedEntry),
											)}
										...
										{#if selectedEntry.happinessRating}
											<div
												class="mt-5 rounded-xl border p-4 {happinessTone(
											...
											>
												...
												>
													<div>
														<p
															class="text-xs font-semibold uppercase tracking-wide opacity-80"
														>
															Overall Happiness
														</p>
														<p
													...
													<div
														class="text-3xl font-bold text-zinc-100"
													>
														{selectedEntry.happinessRating}<span
												...
												<div
													class="mt-4 h-2 overflow-hidden rounded-full bg-zinc-950/50"
												>
													<div
														class="h-full rounded-full bg-current"
														style={`width: ${ratingWidth(selectedEntry.happinessRating)}`}
													></div>
												</div>
											</div>
										{/if}
										{#if selectedEntry.excerpt}
											<p
												class="mt-5 max-w-3xl border-l-2 border-zinc-700 pl-4 text-base leading-relaxed text-zinc-400"
											>
										...
										{#if entryHabitLogs(selectedEntry).length}
											...
											>
												{#each entryHabitLogs(selectedEntry) as log (log.id)}
													<span
														class="rounded-full border border-zinc-800 bg-zinc-950/45 px-3 py-1 text-xs text-zinc-400"
													>
														{log.emoji}
														{log.habitName}
													</span>
						...
						{:else}
							<div
								class="flex flex-col items-center justify-center rounded-2xl border border-zinc-850/60 bg-surface-900/10 px-6 py-24 text-center"
							>
								<div
									class="w-12 h-12 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-500 mb-4"
								>
									<svg
										class="w-6 h-6"
										fill="none"
								...
								<p class="text-xs text-zinc-550 mt-1 max-w-xs">
									Select an entry from the sidebar to read, or
									go to the Admin Dashboard to write a new
									one.
								</p>
							</div>
						{/if}
