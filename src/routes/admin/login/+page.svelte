<script lang="ts">
	...
	async function handleLogin(e: Event) {
		...
		try {
			...
			// Surface a friendlier message for common Firebase errors
...
{#if checkingAuth}
	<!-- Silent full-screen wait while Firebase resolves auth state -->
	<div class="min-h-screen bg-[#09090b] flex items-center justify-center">
		<div class="w-6 h-6 border-2 border-accent-500/30 border-t-accent-500 rounded-full animate-spin"></div>
	</div>
{:else}
	<div class="min-h-screen bg-[#09090b] flex items-center justify-center px-4">
		...
		<div class="relative w-full max-w-sm">
			<!-- Glow orb -->
			<div class="absolute -top-24 left-1/2 -translate-x-1/2 w-48 h-48 bg-accent-600/20 rounded-full blur-3xl pointer-events-none"></div>

			<div class="relative bg-surface-900/80 backdrop-blur-md border border-zinc-800/60 rounded-2xl p-8 shadow-2xl shadow-black/40">
				<!-- Logo / wordmark -->
				<div class="mb-8 text-center">
					<span class="font-mono text-accent-400 text-xs tracking-widest uppercase">&gt;_ mikas</span>
					<h1 class="mt-2 text-xl font-semibold text-zinc-100">Admin Access</h1>
					<p class="mt-1 text-sm text-zinc-500">Sign in to manage your content</p>
				</div>

				<form onsubmit={handleLogin} class="space-y-4" id="login-form">
					<div class="space-y-1.5">
						<label for="email" class="block text-xs font-medium text-zinc-400 tracking-wide uppercase">
							Email
						</label>
						<input
							id="email"
					...
					<div class="space-y-1.5">
						<label for="password" class="block text-xs font-medium text-zinc-400 tracking-wide uppercase">
							Password
						</label>
						<input
							id="password"
					...
					{#if error}
						<div class="flex items-center gap-2 px-3.5 py-2.5 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm">
